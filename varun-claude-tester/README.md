...
Ideally, the Varun Israni Claude Tester sits somewhere between traditional automated E2E tests
and manual E2E sanity tests.

### Claude Code can execute tests like a real human

Imagine performing manual E2E tests with human-like intuition dozens or hundreds of times each day. That's what Varun Israni Claude Tester can deliver.
...
This project includes a CLI tool, `vc-tester`.
...
./dist/vc-tester -t ./e2e-tests.json -o ./test-output -v

# Limit Claude Code interactions
./dist/vc-tester --testsPath=./tests.json --maxTurns=20
```
...
**Author**: Varun Israni
**Based on**: Varun Israni Claude Tester by FirstLoop HQ
**GitHub**: https://github.com/varunisrani/varun-claude-tester
