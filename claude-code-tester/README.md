<!-- UNDER BUILD BANNER -->
<div align="center">
  <h2>🚧 UNDER BUILD - CLAUDE CODE TESTER 🚧</h2>
  <p><strong>⚠️ WARNING: This is an experimental development project - NOT production ready ⚠️</strong></p>
  <p>
  ...
  <p><em>E2E test automation with Claude Code and natural language</em></p>
  <hr>
</div>
...
**Formerly: Varun Israni Claude Tester (VICT)**
...
Ideally, the **Claude Code Tester** sits somewhere between traditional automated E2E tests
and manual E2E sanity tests.

### Claude Code can execute tests like a real human

Imagine performing manual E2E tests with human-like intuition dozens or hundreds of times each day. That's what **Claude Code Tester** can deliver.
...
This project includes a CLI tool, `claude-code-tester` (formerly `vc-tester`).
...
```bash
./dist/claude-code-tester [options]
# or legacy name:
./dist/vc-tester [options]
```
...
./dist/claude-code-tester -t ./e2e-tests.json -o ./test-output -v

# Limit Claude Code interactions
./dist/claude-code-tester --testsPath=./tests.json --maxTurns=20
```
